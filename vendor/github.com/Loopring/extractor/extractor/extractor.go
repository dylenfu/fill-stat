...
func (l *ExtractorServiceImpl) Sync(blockNumber *big.Int) error {
	var syncBlock types.Big
	if err := accessor.BlockNumber(&syncBlock); err != nil {
		return fmt.Errorf("extractor,Sync chain block,get ethereum node current block number error:%s", err.Error())
	}
...
// BlockProcessError 当发生严重错误时需关停extractor,检查错误原因,并重启
func (l *ExtractorServiceImpl) BlockProcessError(err error) {
	l.Stop()
	log.Fatalf("extractor, extremely error, check it out and restart extractor:%s", err.Error())
}
...
func (l *ExtractorServiceImpl) ProcessBlock() error {
	...
	if l.syncComplete == false {
		if err := l.Sync(block.Number.BigInt()); err != nil {
			return err
		}
	}
